...
def _make_fact(
    id_: int, key: str = "k", value: str = "v", confidence: float = 1.0
):
...
# ---------------------------------------------------------------------------
# Fixture: auto-mock _get_cached_area_directory (added in v0.8.0 merge)
# ---------------------------------------------------------------------------


@pytest.fixture(autouse=True)
def _mock_area_directory():
    """Auto-mock _get_cached_area_directory so it doesn't hit HA."""
    with patch(
        "memory.context_builder._get_cached_area_directory",
...
async def test_build_with_user_message_does_semantic_search(builder):
    ...
    # semantic_limit = max(1, max_facts - 5) = max(1, 15) = 15
    builder.knowledge_store.search_semantic.assert_awaited_once_with(
        query="turn on lights", limit=15
    )
...
async def test_build_no_explicit_keyword_fallback(builder):
    ...
    # semantic_limit = max(1, 20 - 5) = 15
    builder.knowledge_store.search_semantic.assert_awaited_once_with(
        query="some query", limit=15
    )
    builder.knowledge_store.search_keyword.assert_not_awaited()


@pytest.mark.asyncio
async def test_build_no_keyword_fallback_when_semantic_has_results(
    builder,
):
...
async def test_build_adds_high_confidence_core_facts(builder):
    ...
    high_conf_fact = _make_fact(
        100, key="fav_color", value="blue", confidence=0.95
    )
...
async def test_build_skips_low_confidence_core_facts(builder):
    ...
    low_conf_fact = _make_fact(
        200, key="maybe", value="unsure", confidence=0.5
    )
...
async def test_build_includes_presence_summary(builder):
    """Presence summary is fetched via _get_cached_presence and forwarded to build_system_prompt."""
    with (
        patch(
            "memory.context_builder._build_time_context",
            return_value=FAKE_TIME_CONTEXT,
...
async def test_build_handles_presence_failure(builder):
    """If _get_cached_presence raises, presence_summary falls back to empty string.

    The cached presence function catches internal exceptions from the underlying
    HA calls, so when it fails it returns an empty string from the cache."""
    with (
        patch(
            "memory.context_builder._build_time_context",
            return_value=FAKE_TIME_CONTEXT,
...
async def test_build_includes_device_summary(builder):
    """Device summary is fetched via _get_cached_device_summary and forwarded to build_system_prompt."""
    with (
        patch(
            "memory.context_builder._build_time_context",
            return_value=FAKE_TIME_CONTEXT,
...
async def test_build_handles_device_failure(builder):
    """If _get_cached_device_summary raises, device_summary falls back to empty string.

    The cached device summary function catches internal exceptions from the
    underlying HA calls, so when it fails it returns an empty string from the cache."""
    with (
        patch(
            "memory.context_builder._build_time_context",
            return_value=FAKE_TIME_CONTEXT,
...
async def test_build_passes_all_context_to_system_prompt(builder):
    ...
    with (
        ...
        patch(
            "memory.context_builder._get_cached_presence",
            new_callable=AsyncMock,
            return_value="Salih: home",
        ),
        patch(
            "memory.context_builder._get_cached_device_summary",
            new_callable=AsyncMock,
            return_value="light.office",
        ),
        patch(
            "memory.context_builder.fetch_service_schemas",
            new_callable=AsyncMock,
...
async def test_build_invalid_timezone_falls_back_to_utc(builder):
    ...
    now_arg = mock_tc.call_args[0][0]
    assert now_arg.tzinfo == datetime.UTC
