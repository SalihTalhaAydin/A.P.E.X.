...
@pytest.mark.asyncio
async def test_control_area_empty_area_name_rejects_bug_102():
    """Bug 102: Empty area_name must not match first area in list."""
    captured_payload = {}
    ...
    with patch(
        ...
        result_empty = await control_area(
            area_name="",
            action="on",
            domain="light",
        )
        result_whitespace = await control_area(
            area_name="   ",
            action="on",
            domain="light",
        )

    # Must return error, not control first area
    assert "cannot be empty" in result_empty.lower()
    assert "cannot be empty" in result_whitespace.lower()
    # Must NOT have called service (no area_id captured)
    assert captured_payload.get("area_id") is None
