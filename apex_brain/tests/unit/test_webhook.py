...
async def test_process_event_processes_real_change():
    ...
    # BUG-63/64: Each event must get a unique session_id so histories don't bleed
    call_kw = mock_convo.handle.call_args.kwargs
    session_id = call_kw["session_id"]
    assert session_id.startswith("apex_events")
    assert "binary_sensor.hallway_motion" in session_id


@pytest.mark.asyncio
async def test_bug63_bug64_unique_session_id_per_event():
    """Each webhook event gets a unique session_id (no history bleeding)."""
    mock_convo = AsyncMock()
    mock_convo.handle.return_value = "OK"
    ...
    await handler.process_event(event1)
    await handler.process_event(event2)

    assert mock_convo.handle.call_count == 2
