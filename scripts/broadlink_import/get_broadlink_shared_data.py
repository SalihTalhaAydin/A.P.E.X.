...
def load_json(name: str) -> dict | list:
    ...
    if not os.path.exists(path):
        ...
        print(
            "  Get it from e-Control: Share → Share to other phones in WLAN"
        )
        sys.exit(1)
...
def main() -> None:
    ...
    if "--repeat" in sys.argv:
        ...
        sys.argv = [
            a for i, a in enumerate(sys.argv) if i not in (idx, idx + 1)
        ]
    ...
    accessory_name = (
        selected.get("name", "unknown").replace("/", "-").replace(" ", "_")
    )
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        for code_entry in json_code:
            ...
            hex_str = (
                hex_str * repeat
            )  # for TC2 RF switches that need duplication
            ...
            hex_preview = (
                hex_str[:48] + "..." if len(hex_str) > 48 else hex_str
            )
            line = f"Button: {btn_name} | Hex: {hex_preview} | Base64: {b64}\n"
            f.write(line)
            results.append(
                {"name": btn_name, "hex": hex_str, "base64": b64}
            )
